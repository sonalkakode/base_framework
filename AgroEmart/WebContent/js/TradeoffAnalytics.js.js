 ...
 * 
 * AUS720150066 Â© Copyright IBM Corp. 2015 All Rights Reserved
 * 
...
(function(){
  ...
  //      if (this.importantEvents.indexOf(event.data.type) >= 0) {
  ...
  TradeoffAnalytics = TA.TradeoffAnalytics ;//backward compitability
  ...
  createCssLink = function(href, id, onload) {
    var link;
    if (document.createStyleSheet) { // old IE
      ...
      link = createDom('link', {
        ...
        rel : 'stylesheet',
        type : 'text/css',
        onerror : function(e){throw 'The stylesheet ' + e.target.src + ' is not accessible.'},
        onload : onload
      }, document.head);
    }
    if (id) {
  ...
  createScriptLink = function(src, onload) {
    var head = document.querySelector('head');
    var script = createDom('script', {
      type : 'text\/javascript',
      src : src,
      onerror : function(e){throw 'The script ' + e.target.src + ' is not accessible.'},
      onload : onload
    }, head);
  };
  createDom = function(elem, map, parent) {
  ...
  xhr = function(url, method, headers, body, okCallback, errCallback, username, password) {
